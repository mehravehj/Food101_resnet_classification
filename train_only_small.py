'''
Sanity check
train and overfit for only one batch of data
'''
import argparse
import os
import random
...
def main_worker(gpu, args):
    ...
    if args.pretrained:
        ...
        print('warning pretrained model')
    else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        train(images, targets, model, criterion, optimizer, scaler, epoch, args)
        # with torch.no_grad():
...
def train(images, target, model, criterion, optimizer, scaler, epoch, args):
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    progress = ProgressMeter(256, [batch_time, losses, top1],
